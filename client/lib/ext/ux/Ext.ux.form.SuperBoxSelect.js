...
Ext.ux.form.SuperBoxSelect = Ext.extend(Ext.ux.form.SuperBoxSelect,Ext.form.ComboBox, {
	...
	/**
	 * @cfg {Boolean} true erzwingt die Auswahl des ersten Elements und verhindert das entfernen
	 */
	...
	/**
	 * Interne Marke hält fest, ob das Dialogelement gerendert wurde
	 */
	isrendered:			false,
	
	/**
	 * Interne Marke hält fest, ob ein selectfirst nach dem rendern ausgeführt werden soll
	 */
	selectfirstonrender: false,
	
	/**
	 * Konstruktor - Initialisierungssequenz
	 */
	initComponent:function() {
		...
		// Wenn multiselect, dann forceSelection auf false setzen.
		if(this.multiselect) this.forceSelection = false;
		if(this.transform){
			this.doTransform();
		}
		...
		/*
		 * forceSelection:		store.load hooked auch bei 'local'-mode
		 */
		if(this.forceSelection && this.store) {
			this.store.on('load', this.selectFirstElement, this);
		}
	},
	onRender:function(ct, position) {
		...
		/*
		 * Single-Select rendert das Inputfield sichtbar
		 */
		var inputClass = 'x-superboxselect-input-field';
		if(!this.multiselect)			inputClass += ' x-superboxselect-singleselect';
		this.el.removeClass('x-form-text').addClass(inputClass);
		...
		if(this.forceSelection)		this.selectFirstElement();
		if(this.mode === 'remote' && this.forceSelection) {
			if(!this.store.isloaded)			this.store.load();
		}
		
		this.isrendered = true;
		if(this.selectfirstonrender) {
			this.selectfirstonrender = false;
	...
	/**
	 * Wähle den ersten Datensatz, sofern das Element noch leer ist (und der Store über Datensätze verfügt)
	 */
	selectFirstElement:function() {
		if(!this.isrendered) {
			this.selectfirstonrender = true;
			return;
		}
		if(this.getValue() != '')					return;
		if(this.store.getCount() == 0)		return;
		this.addRecord(this.store.getAt(0));
	},
	...
	onKeyDownHandler : function(e,t) {
		...
		/*
		 *  Entferne nur wenn forceSelection = false oder multiselect = true.
		 */
		if(e.getKey() === e.BACKSPACE && (!this.forceSelection || this.multiselect )){
			e.stopEvent();
			if(this.currentFocus) {
				toDestroy = this.currentFocus;
	...
	addRecord : function(record) {
		...
		/**
		 * Erzeugt ein Element für die Auswahl
		 */
		addItemBox : function(itemVal,itemDisplay,itemCaption, itemClass, itemStyle) {
			...
			box = new Ext.ux.form.SuperBoxSelectItem({
				...
				value:					itemVal,
				key:						itemKey,
				multiselect:		this.multiselect,				//	multiselect-Kennzeichen übermittelt dem Item, ob es für single- oder multiselect gerendert werden soll
				listeners: {
		...
		manageClearBtn : function() {
			if(!this.renderFieldBtns || !this.rendered) 		return this;
			...
			if (this.items.getCount() === 0) {
				...
				/*
				 * Clear-Button wird ausgeblendet, falls forceSelection true ist
				 */
				if(this.multiselect || !this.forceSelection)				this.buttonClear.removeClass(cls);
			}
			return this;
		},
		...
		onSelect : function(record, index) {
			if (this.fireEvent('beforeselect', this, record, index) !== false){
						...
						if(this.fireEvent('beforeadditem',this,val) !== false) {
								/*
								 * Single-Select: remove Elements before adding
								 */
								if(!this.multiselect) {
									this.setValue('');
								}
		...
		doQuery : function(q, forceAll,valuesQuery, forcedAdd) {
			/*
			 * Für eine effizientere Teilanfrage an den Server wird this.lastquery (in expliziter Kleinschreibweise - nicht zu verwechseln mit this.lastQuery vom Original)
			 * verwendet, um den letzten Query zu merken. Verändert dieser sich durch Erweiterung des Suchfilters, dann reicht die bestehende Ergebnisliste
			 */
				q = Ext.isEmpty(q) ? '' : q;
				var qe = {
						query: q,
				...
				if(this.lastQuery == '' && q == '' && !this.store.isloaded)		this.store.load();
				if(forceAll === true || (q.length >= this.minChars || this.lastquery.indexOf(q) === 0) || valuesQuery && !Ext.isEmpty(q)){
						if(this.lastQuery !== q || forcedAdd){
							this.lastQuery = q;
								if(this.mode == 'local'){
									 ...
									 */
									if(this.lastquery != '' && q.indexOf(this.lastquery) === 0) {
										/*
										 * Lösche alle Inhaltselemente vom Store, die NICHT dem query entsprechen
										 */
										for(var i=this.store.getCount()-1; i>=0; i--) {
											if(this.store.getAt(i).data.value.toUpperCase().indexOf(q.toUpperCase()) <0 )			this.store.removeAt(i);
										}
									} else {
										this.store.load({
...
Ext.ux.form.SuperBoxSelectItem = Ext.extend(Ext.ux.form.SuperBoxSelectItem,Ext.Component, {
	...
	onRender:				function(ct, position) {
		...
		/*
		 * Single-Select unterdrückt den Close-Button
		 */
		if(this.multiselect) {
			var cfg = {
				tag: btnEl,
		...
		this.keyMap = new Ext.KeyMap(this.lnk, [
			{
				...
				fn:							function() {this.moveFocus('right');},
				scope:					this
			}, {
				key:						[Ext.EventObject.LEFT,Ext.EventObject.UP],
				fn:							function() {this.moveFocus('left');},
				scope:					this
			}, {
				key:						[Ext.EventObject.HOME],
				fn:							function() {var l = this.owner.items.get(0).el.focus(); if(l) l.el.focus();},
				scope:					this
			}, {
	...
	preDestroy:			function(supressEffect) {
		if(this.fireEvent('remove', this) === false) 			return;
