...
//////////////////////////
	...
	/**	$_tables_loaded										true, when tables are loaded
		*	@var	boolean
		*/
	...
	/**	$_in_trx													ture, if in transaction
		*	@var	boolean
		*/
...
//	Public methods	//
//////////////////////
	/**	__construct($dsn, $looger)				Creates an instance of Ruckusing_Adapter_MySQL_Base
	...
	/**	get_database_name()								Get the current db name
		*	@return	string
		*/
		public function get_database_name() {
			return($this->db_info['database']);
		}
	/**	supports_migrations()							Check support for migrations
		*	@return	boolean
		*/
		public function supports_migrations() {
			return true;
		}
	/**	native_database_types()						Get the column native types
		*	@return	array
		*/
	...
	/**	create_schema_version_table()			Create the schema-table, if necesssary	*/
		...
		*/
		public function set_current_version($version)  {
			if (defined('APP_BRANCH_DO_MIGRATE')) {
				return $this->execute_ddl("UPDATE ".RUCKUSING_SCHEMA_TBL_NAME." SET version='$version' WHERE branch='" . APP_BRANCH . "'");
			} else {
				try {
					$ret = $this->execute_ddl("UPDATE ".RUCKUSING_SCHEMA_TBL_NAME." SET version='$version' WHERE branch=''");
					return $ret;
				} catch (Exception $e) {
// 					if (!is_int(stripos($e->getMessage(), "Unknown column"))) {
// 						throw $e;
// 					}
					$ret = $this->execute_ddl("UPDATE ".RUCKUSING_SCHEMA_TBL_NAME." SET version='$version'");
					return $ret;
				}
			}
		}
	/**	remove_version($version)					Remove a version
	...
	/**	__toString()											Return a message displaying the current version
		*	@return	string
		*/
...
//	Private methods	//
//////////////////////
	/**	connect($dsn)											Connect to the db
		*	@param	string	$dsn							The current dsn
		*/
		private function connect($dsn) {
			$this->db_connect($dsn);
		}
	/**	db_connect($dsn)									Connect to the db
		*	@param	string	$dsn							the current dsn
		*	@throws	Ruckusing_Exception
		*	@return	boolean
		*/
		private function db_connect($dsn) {
			...
			if($db_info) {
				...
				if(empty($db_info['port']))					$db_info['port']			=	3306;
				if(empty($db_info['socket']))				$db_info['socket']		=	@ini_get('mysqli.default_socket');
				if (empty($db_info['charset']))			$db_info['charset']		=	"utf8";
	...
	/**	load_tables($reload=true)					Initialize an array of table names
	...
	/**	determine_query_type($query)			Check query type
		*	@param	string	$query						Query to run
		*	@return	int
		*/
		private function determine_query_type($query) {
			...
			$type = $match[0];
			switch ($type) {
				case 'select':						return SQL_SELECT;								break;
				case 'update':						return SQL_UPDATE;								break;
				case 'delete':						return SQL_DELETE;								break;
				case 'insert':						return SQL_INSERT;								break;
				case 'alter':							return SQL_ALTER;									break;
				case 'drop':							return SQL_DROP;									break;
				case 'create':						return SQL_CREATE;								break;
				case 'show':							return SQL_SHOW;									break;
				case 'rename':						return SQL_RENAME;								break;
				case 'set':								return SQL_SET;										break;
				default:									return SQL_UNKNOWN_QUERY_TYPE;		break;
			}
		}
	/**	is_select($query_type)						Check Query is select
	...
	/**	is_sql_method_call($str)					Detect wheter or not the string represents a function call and if so,
		*																		do not wrap it in single-quotes, otherwies do wrap in single quotes.
		*	@param	string	$str
		*	@return	boolean
		*/
	...
	/**	inTransation()										Check if in transaction
		*	@return boolean
		*/
